...
func main() {
	port := envFlagInt("GOPERFORMERPORT", "port", 8060,
		"The port to use")
	version := envFlagString("GOVERSION", "version", "1.0.0",
		"The version to use")
	...
	performer := NewInsightsPerformer(logger)
	protocol.RegisterColumnarServiceServer(grpcSrv, performer)
	protocol.RegisterColumnarCrossServiceServer(grpcSrv, performer)

	logger.Logf(logrus.InfoLevel, "Starting grpc server for Go Operational Insights at %d", *port)
